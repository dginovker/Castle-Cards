...
func _process_leadership_support(delta: float) -> void:
    ...
    for previous_key: Variant in _leadership_applied_targets.keys():
        ...
        var previous_unit: CombatUnit = _leadership_applied_targets.get(previous_key, null) as CombatUnit
        if previous_unit != null and is_instance_valid(previous_unit):
            previous_unit.clear_leadership_bonus_from_source(leadership_kind, self)
...
func _clear_leadership_from_previous_targets() -> void:
    for unit_variant: Variant in _leadership_applied_targets.values():
        var unit: CombatUnit = unit_variant as CombatUnit
        if unit == null or not is_instance_valid(unit):
            continue
        unit.clear_leadership_bonus_from_source(leadership_kind, self)
