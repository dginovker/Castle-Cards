...
func _ensure_hurtbox() -> void:
    ...
    circle_shape.radius = maxf(8.0, target_render_height * 0.24)

    _hurtbox_area.monitoring = true
    _hurtbox_area.monitorable = true
    _hurtbox_area.collision_layer = 0
    _hurtbox_area.collision_mask = 0
    _hurtbox_area.set_collision_layer_value(GameConstants.COMBAT_HURTBOX_LAYER, true)
...
func _process_searching_tree(delta: float) -> void:
    var closest_tree_ahead = _find_first_fully_grown_tree_towards_offset(_get_enemy_side_offset())
...
func _process_returning_empty(delta: float) -> void:
    ...
    var closest_tree_ahead = _find_first_fully_grown_tree_towards_offset(own_offset)
    if closest_tree_ahead != null:
        _target_tree = closest_tree_ahead
        _target_tree_offset = _get_tree_lane_offset(_target_tree)
...
func _begin_chopping(tree) -> void:
...
func _find_first_fully_grown_tree_towards_offset(target_offset: float):
    if lane_curve == null:
        return null

    var best_tree
    var best_distance_ahead: float = INF
    var direction_sign: float = 1.0 if target_offset >= current_offset else -1.0
...
func _get_tree_lane_offset(tree) -> float:
    if tree.has_meta("lane_offset"):
        var lane_offset: float = tree.get_meta("lane_offset")
        return lane_offset
...
func _is_target_tree_still_valid(tree) -> bool:
    if tree == null:
        return false
