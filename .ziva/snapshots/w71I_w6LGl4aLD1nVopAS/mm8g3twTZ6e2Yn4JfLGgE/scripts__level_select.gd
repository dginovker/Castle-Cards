...
func _ready() -> void:
    ...
    # Check if level 4 is beaten to unlock shores
    var forest_complete = GameState.is_level_beaten("level_4")
    shores_button.disabled = not forest_complete
    
    if shores_button.disabled:
        shores_button.text = "Shores (Locked)"
        shores_button.modulate = Color(0.5, 0.5, 0.5, 1)
    else:
