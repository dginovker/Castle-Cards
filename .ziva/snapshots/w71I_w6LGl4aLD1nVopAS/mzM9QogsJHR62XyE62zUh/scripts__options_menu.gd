...
func _refresh_stats(gs) -> void:
    if gs:
        highest_level_label.text = "Highest Level: " + gs.get_highest_level_beaten()
        wins_label.text = "Total Wins: " + str(gs.total_wins)
        losses_label.text = "Total Losses: " + str(gs.total_losses)
