...
func _resume_web_audio() -> void:
    _audio_resumed = true
    # Ensure the Master bus is unmuted and the server is definitely active
    AudioServer.set_bus_mute(0, false)
    
    # Even if it says it's "playing", we stop and restart to force the buffer to fill
    # with the newly resumed AudioContext.
    var current_pos = music_player.get_playback_position()
    music_player.stop()
    music_player.play(current_pos)
    
    print("MusicPlayer: Web Audio context resumed successfully.")
