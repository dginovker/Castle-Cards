...
func _setup_square_button(btn: Button, type: String) -> void:
    ...
    style_disabled.modulate_color = Color(0.7, 0.7, 0.7, 1)
    ...
    if icon_rect and icon_rect is TextureRect:
        ...
        # Add a light backer to make icons pop on dark backgrounds
        var backer = ColorRect.new()
        backer.name = "IconBacker"
        backer.color = Color(1, 1, 1, 0.15)
        backer.custom_minimum_size = Vector2(70, 70)
        backer.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
        backer.size_flags_vertical = Control.SIZE_SHRINK_CENTER
        backer.mouse_filter = Control.MOUSE_FILTER_IGNORE
        icon_rect.add_child(backer)
        icon_rect.move_child(backer, 0) # Put behind icon
        backer.set_anchors_and_offsets_preset(Control.PRESET_CENTER)
...
func _update_tree_growth_button() -> void:
    ...
    var spawn_interval = GameState.get_current_tree_growth_interval(4.0)
    
    if not is_lvl4_beaten:
        name_label.text = "Faster Spawning\n(Beat Lvl 4 to unlock)"
        if status_label:
            status_label.text = "Spawn: %0.1fs" % spawn_interval
