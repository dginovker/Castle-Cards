...
func _ready() -> void:
    ...
    for btn in [archer_button, drummer_button, cannon_button, tree_growth_button, back_button]:
...
func _update_tree_growth_button() -> void:
    var is_unlocked = GameState.is_level_beaten("level_4")
    tree_growth_button.visible = is_unlocked
    if not is_unlocked:
        return
        
    var cost = GameState.get_tree_growth_upgrade_cost()
    var action_label: Label = tree_growth_button.get_node("Content/ActionLabel")
    var cost_container: Control = tree_growth_button.get_node("Content/CostContainer")
    var cost_label: Label = cost_container.get_node("CostLabel")
    
    action_label.text = "Faster Tree Growth (Lvl %d)" % GameState.tree_growth_upgrades
