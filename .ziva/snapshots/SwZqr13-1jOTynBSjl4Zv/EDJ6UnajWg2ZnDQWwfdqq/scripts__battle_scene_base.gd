...
func _refresh_wood_ui() -> void: if player_wood_value_label != null: player_wood_value_label.text = str(_player_wood); player_wood_value_label.tooltip_text = ("Woodcutters deliver %d wood/tree (base %d + bonus %d)." % [GameConstants.WOODCUTTER_DELIVERY_WOOD + GameState.get_tree_yield_bonus_per_tree(), GameConstants.WOODCUTTER_DELIVERY_WOOD, GameState.get_tree_yield_bonus_per_tree()]); if enemy_wood_value_label != null: enemy_wood_value_label.text = str(_enemy_wood)
